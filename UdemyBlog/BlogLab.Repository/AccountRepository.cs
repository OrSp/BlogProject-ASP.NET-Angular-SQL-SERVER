...
namespace BlogLab.Repository
{
    public class AccountRepository : IAccountRepository
    {
        ...
        public async Task<ApplicationUserIdentity> GetByUsernameAsync(string normalazedUsername, CancellationToken cancellationToken)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ...
            using (var connection = new SqlConnection(_config.GetConnectionString("DefaultConnection")))
            {
                ...
                applicationUser = await connection.QueryFirstOrDefaultAsync<ApplicationUserIdentity>(
                    "Account_GetByUsername", new { NormalizedUsername = normalazedUsername }, commandType: CommandType.StoredProcedure
                    );
            }
